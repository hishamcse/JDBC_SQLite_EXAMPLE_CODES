...
public class Datasource {
    ...
    public List<SongArtist> queryartistforsongs(String title, int order) {
        String string = QUERY_ARTISTS_FOR_SONG + "'" + title + "'";
        StringBuilder sb = new StringBuilder(string);
        if (order != ORDER_BY_NONE) {
    ...
    public List<SongArtist> view_by_query(String title) {

        String s = QUERY_FROM_VIEW + "'" + title + "'";
        try (Statement statement = connection.createStatement();
    ...
    public List<SongArtist> view_by_query_prepared(String title) {
        try {
            preparedStatement.setString(1, title);  //first ? is 1
